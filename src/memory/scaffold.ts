...
- \`research\` (optional; absent or \`enabled: false\` means disabled): \`enabled\`, \`provider\` (\`jlcsearch\` by default or \`nexar\`), \`searchProvider\` (\`none\` by default or \`brave\`), \`allowHosts\` (host allowlist), \`stalenessDays\` (default 30), and \`maxPdfMB\` (default 10). The default JLCSearch provider needs no key; optional Brave/Nexar keys stay in environment variables. \`check\` never uses them or makes network calls.

## constraints.json

Machine-readable constraint registry. Each key maps to \`{ min/max/forbidden/value, source, affects[] }\`; sourcing snapshots additionally carry \`mpn/lifecycle/stockTotal/price1k/retrieved\`.
...
Committed research evidence: PDF cache files, per-page extracted text, and
\`index.json\`. Keep the cache reviewable and reproducible; if it grows large,
move the PDFs to Git LFS while keeping \`index.json\` and the extracted text in
the normal repository. \`copperhead check\` validates cached hashes offline.
`;
}
