...
- **AC-15.23 / AC-15.24 (content-aware completion)** The schematic stage completes only with symbols present and drift-clean BOM/PINOUT (layout-draft: a board with a footprint plus the LAYOUT.md marker); finish refuses while the stage contract is unmet, halting the pipeline for resume instead of advancing.
...
- **AC-16.37 (local runs are wired)** For every signal net, each cluster of endpoints within one group and within wire span of one another that routes cleanly is drawn as one wired run with one label, whatever the rest of the net does; a cluster that cannot be routed whole is drawn as its largest routable subset. A cluster of more than four endpoints is first narrowed to its largest set of endpoints bound to one anchor (a pin and the parts hung on it), the rest keeping stubs and labels. A net that leaves the group therefore still wires the part on its pin, however many parts hang there.
...
- **AC-16.45 (text keeps 0.8 mm of air)** Every engine clearance check that places a net label or a part's Reference or Value against other text, and the final sweep that settles power-symbol values, pads the candidate by 0.8 mm. Two relaxations are deliberate: a wired run's label with no padded point anywhere on its run takes the first point clear with a quarter pad, then with none, rather than stay on a body; and a power stub's preferred length is chosen against other rails' values unpadded, since the final sweep settles them. Outside those, two texts whose boxes touch are a collision.
...
- **AC-16.47 (group boxes never intersect)** When two drawn group boxes intersect, a box crosses the frame, or a box has grown into the title block's corner, the sheet is re-tiled and drafted again: an overlap along a line of the wrap widens the right-hand group's tiling reserve, an overlap across the wrap widens the gap between wrapped lines, and an overflow shrinks the usable frame, each by the overlap plus a unit. At most three re-tilings per round and sixteen drafts in all, deterministically; a draft kept with boxes still wrong names them in its notes.
...
- **AC-16.50 (the title block is a corner)** Content may extend into the title strip when every group box that reaches into it stays clear of the title block's own corner (110 mm wide at the bottom right); the sheet fit judges this on the group rects it wraps, and a box that has grown into the corner by the time the sheet is drawn is re-drafted as an overflow (AC-16.47).
...
- **AC-16.53 (the squeeze)** After the first draft the engine measures each cell's drawn extent (body, fields, and the wires, labels and power symbols of the runs on its pins, hung parts rolled up to their anchor; shared runs attributed only through the wires touching the cell's own pins), drafts again with cells sized from those measurements plus one unit of pad, and keeps a round only when the group boxes shrink by more than three percent with no more refused hangs than the kept draft, no larger sheet, no merged nets and the label-overlap budget kept; at most three rounds, and only when `COPPERHEAD_DRAFT_SQUEEZE=1` asks for them. `sheetFit.squeeze` always reports the rounds drafted and the box area before and after.
...
- **AC-16.61 (boxes align)** Group boxes in one row of the wrap share the row's bottom edge and boxes in one column the column's right edge, except a box whose shared edge would enter the title block's corner, which keeps its own width or height there.
- **AC-16.62 (masonry wrap)** When neither rows nor columns in declared order fit a sheet, groups are dealt to k columns (two to four), each column stacking its own groups in declared order with columns opened in reading order; the narrowest deal that fits the usable height is taken, ties to the shorter. The deals are searched in reading order, cutting only branches that cannot beat the best deal found so far, so the search keeps the deal a full enumeration keeps; past 200 000 search nodes it keeps the best deal found by then, or deals the groups round-robin when it has none. The draft report names the shape, with k the number of columns drawn (`groups laid in k columns … each column stacks its own groups`).
