...
- `vertex.ts`: Claude via Google Cloud Vertex AI, authenticated by Application Default Credentials (never `ANTHROPIC_API_KEY`). Selected by `--model vertex` / `vertex:<id>` (matched as its own namespace); project/region resolve `COPPERHEAD_VERTEX_PROJECT`/`COPPERHEAD_VERTEX_REGION` > `vertexProject`/`vertexRegion` in config > the Vertex SDK's own `ANTHROPIC_VERTEX_PROJECT_ID`/`CLOUD_ML_REGION`; region defaults to `global`, a missing project fails fast. Bare `vertex` uses the same default model id as `claude` (one shared constant). Model ids pass through verbatim except the Anthropic-style dated form (`-YYYYMMDD`), rejected before any network call naming the Vertex `@YYYYMMDD` form. The request/response shape (cache breakpoints, block mapping, token accounting) is shared with `anthropic.ts` via one module, so the routes cannot diverge. `@anthropic-ai/vertex-sdk` ships as an `optionalDependency`, lazily imported only when `vertex` is selected; never a keyed-failover candidate and never auto-selected.
...
- Rate-limit (429): exponential backoff ×3, then fail over to the other **keyed** provider (`openai` ↔ `anthropic`) if a key exists; saved-login providers (`codex`, `claude-code`, `cursor`), the compat provider, and the Vertex provider never fail over to a keyed or alternate provider — for Vertex, failing over would silently move a run off the GCP project carrying its billing, quota, audit trail, and residency commitment
...
- **AC-3.10 (provider parity)** AC-3.1 passes with `--model codex`, `--model gpt-5`, `--model claude`, `--model claude-code`, `--model cursor`, `--model compat:<id>`, and `--model vertex:<id>` when each provider is configured.
...
- **AC-3.21 (vertex ADC auth)** A `--model vertex:<id>` run authenticates via Google Application Default Credentials and neither reads nor requires `ANTHROPIC_API_KEY`; a set key changes nothing about the request.
- **AC-3.22 (vertex project/region resolution)** Project and region resolve env > config > the Vertex SDK's own variables; region defaults to `global`; a missing project fails before any network call with a message naming every setting that could supply one.
- **AC-3.23 (vertex model-id form)** `vertex:<id>` passes the id through verbatim; bare `vertex` uses the same default model as `claude`; `vertex:` (empty) is rejected; an Anthropic-style dated id (`-YYYYMMDD`) is rejected before any network call naming the Vertex `@YYYYMMDD` form.
- **AC-3.24 (vertex shares the Anthropic request shape)** For the same conversation and tools, the Vertex request body is identical to the first-party Anthropic route's (same three cache breakpoints, same block mapping, same cache-inclusive token accounting), built through one shared module.
- **AC-3.25 (vertex never fails over to a keyed provider)** A rate-limited Vertex run fails rather than switching to `OPENAI_API_KEY`/`ANTHROPIC_API_KEY`: the provider's name is `'vertex'`, distinct from both keyed providers, extending the AC-3.18 isolation.
- **AC-3.26 (vertex is opt-in only)** `resolveModel()` never auto-selects the vertex route; exported Vertex settings or GCP credentials do not make it a fallback candidate.
- **AC-3.27 (vertex doctor)** `doctor --model vertex` reports the resolved project and region and whether an ADC source is discoverable offline (`GOOGLE_APPLICATION_CREDENTIALS` file, gcloud ADC file, or metadata-server environment), failing with an actionable hint when either is missing; it makes no network request (no token minting, no Model Garden check). Its privacy line is a non-blocking `info` naming the project's Google Cloud terms — the `generativelanguage.googleapis.com` free-tier `warn` (AC-3.17) does not carry over to Vertex.

### AC-4 · Safety

- **AC-4.1** No file in the repo, transcript, or any commit ever contains a string matching `sk-[A-Za-z0-9_-]{20,}` (grep the whole tree + `.copperhead/runs/` after all tests). Write-time redaction also covers Google credential shapes for the Vertex route: `ya29.`-prefixed OAuth access tokens and PEM private-key blocks (a service-account JSON's `private_key`); the ADC credential itself is held and refreshed by Google's auth library — copperhead handles only project and region and never reads a credential file it did not create.
