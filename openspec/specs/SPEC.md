...
- **AC-3.21 (the re-sent conversation is capped)** Every provider request carries a capped view of the conversation rather than the full accumulated history: a `read_file` result whose path is read again later by a read whose line span contains it is replaced with a short stub, and a settled tool result or already-applied tool-call argument over its cap is clipped head and tail with an in-band marker naming how to recover it. Message count, order, roles, and `toolCallId` values are identical to the uncapped history, so session-resume indexing and tool-result pairing are unaffected. The run's own history, the JSONL transcript, and the obligations ledger keep full fidelity; capping is on by default and `historyCap: false` disables it, sending the conversation exactly as accumulated.
- **AC-3.22 (capping never invents coverage)** A read is credited only with the span the tool actually returned: `read_file` line bounds are normalized to a finite number or an absent bound before the span is computed, so a bound supplied as a numeric string is not mistaken for a whole-file read. A partial read never supersedes a read it does not contain, and a failed read never supersedes a successful one. The newest read of a path is never superseded, though it is still subject to result truncation once outside the recent window.
