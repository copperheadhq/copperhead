...
- **Hidden fields stay hidden in KiCad 10 files.** eeschema 10 writes `(hide yes)` as a direct child of a property; the reader takes that as hidden like the older form inside the effects block, so a hand-edited sheet's power references do not read as text on a wire.
